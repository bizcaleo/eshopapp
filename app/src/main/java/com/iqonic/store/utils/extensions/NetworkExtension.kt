...
import com.iqonic.store.ProShopApp.Companion.getAppInstance
import com.iqonic.store.ProShopApp.Companion.restApiImpl
...
import com.iqonic.store.utils.Constants.SharedPref.IS_SOCIAL_LOGIN
...
fun AppBaseActivity.createCustomer(
    ...
    if (isNetworkAvailable()) {
        ...
        getRestApiImpl().CreateCustomer(requestModel, onApiSuccess = {
            showProgress(false)
            if (it.code == 200) {
                getSharedPrefInstance().setValue(USER_DISPLAY_NAME, it.data.display_name)
                getSharedPrefInstance().setValue(USER_EMAIL, it.data.user_email)
                getSharedPrefInstance().setValue(USER_FIRST_NAME, it.data.display_name)
                getSharedPrefInstance().setValue(USER_LAST_NAME, it.data.display_name)
                onApiSuccess(it)
...
fun AppBaseActivity.saveProfileImage(requestModel: RequestModel, onApiSuccess: (Boolean) -> Unit) {
    if (isNetworkAvailable()) {
        showProgress(true)
        getRestApiImpl().saveProfileImage(requestModel, onApiSuccess = {
            showProgress(false)
